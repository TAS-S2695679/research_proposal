...
background_counts <- matched_background %>%
  count(gene_biotype, length_bin, expression_bin, name = "background_count")

# Compare to BDP bin counts
bin_check <- background_counts %>%
  full_join(bin_counts, by = c("gene_biotype" = "biotype_bin", 
                               "length_bin", 
                               "expression_bin")) %>%
  mutate(
    bdp_count = n,
    match = background_count == bdp_count
  ) %>%
  filter(!match)
...
custom_valid_universe <- bitr(union(bdp_genes, matched_background$ensembl_gene_id),
     fromType = "ENSEMBL", toType = "SYMBOL", OrgDb = org.Mm.eg.db)$SYMBOL
